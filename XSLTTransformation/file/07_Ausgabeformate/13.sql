...
v_Teilnehmer2 t_Teilnehmer;
v_Warteliste2 t_Teilnehmer;
...
BEGIN
 ...
 -----------------------------
 SELECT TN_Vorname, TN_Nachname
   BULK COLLECT INTO v_VOrnamen, v_Nachnamen
  FROM teilnehmer
  SAMPLE (0.55);
  
 FOR i IN 1..v_Vornamen.COUNT LOOP
  IF i <= 6
   THEN v_Teilnehmer2(i).TN_Vorname := v_Vornamen(i);
        v_Teilnehmer2(i).TN_Nachname := v_Nachnamen(i);
  ELSE
    v_Warteliste2(i).TN_Vorname := v_Vornamen(i);
    v_Warteliste2(i).TN_Nachname := v_Nachnamen(i);
  END IF;
 END LOOP;
END;