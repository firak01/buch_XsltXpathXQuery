  ...
  <xsl:template match="/Umsatz-Pro-Stadt">
    <html>
      ...
      <body>
        ...
        <ul>
          <li>
            <xsl:value-of select="('Differenz Tage: ' , days-from-duration(subtract-dateTimes-yielding-dayTimeDuration($MaxDateTime, $MinDateTime)))"/>
          </li>
          <li>
            <xsl:value-of select="('Differenz Monate: ' , months-from-duration(subtract-dateTimes-yielding-yearMonthDuration($MaxDateTime, $MinDateTime)))"/>
          </li>
        </ul>
        <h2>Basis xs:date</h2>
        <ul>
          <li>
            <xsl:value-of select="('Differenz Tage: ' , days-from-duration(subtract-dates-yielding-dayTimeDuration($MaxDate, $MinDate)))"/>
          </li>
          <li>
            <xsl:value-of select="('Differenz Monate: ' , months-from-duration(subtract-dates-yielding-yearMonthDuration($MaxDate, $MinDate)))"/>
          </li>
        </ul>
      </body>
    </html>
